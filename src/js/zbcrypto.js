...
function computeEvpKDF(_pwd, _salt, _keySize, _ivSize){
	...
	var keydat = new forge.util.ByteBuffer();
	/** @type {string} */
	var block = "";
...
function zbDataCrypto(_encFlg, _dat, _cfg){
	...
	/** @type {forge.util.ByteBuffer} */
	var key2 = new forge.util.ByteBuffer(cfg.key);
	...
	cipher.update(new forge.util.ByteBuffer(_dat));
	cipher.finish();
...
function ZbCryptoReader(_info){
	...
	this.prepare = async function(offset){
		...
		if(startPos){
			// read for decrypt
			/** @type {forge.util.ByteBuffer} */
			var ivdat = await this._read(this.BLOCK_SIZE);
			iv2 = ivdat.getBytes();
		}

		/** @type {forge.util.ByteBuffer} */
		var key2 = new forge.util.ByteBuffer(this.key);
		if(this.encrypt){
	...
	this.read = async function(offset, size){
		...
		if(startPos < this.reader.getPos()){
			/** @type {number} */
			var st = startPos - (this.reader.getPos() - wlen);
			if(st > 0){
	 ...
	 * @private
	 * @param {number=} size
	 * @return {!Promise<forge.util.ByteBuffer>}
	 */
	this._read = async function(size){
		/** @type {ArrayBuffer} */
		var buf = await this.reader.read(size);
		return new forge.util.ByteBuffer(buf);
	};
}
